...
def find_jvm_library(java_home: str) -> Optional[str]:
    """
...
def find_mpxj_jar() -> Optional[str]:
    """
...
def check_mpxj_availability() -> Dict[str, Any]:
    ...
    try:
        ...
        result["modules"]["mpxj_package"] = f"ERROR: Not installed"
    ...
    try:
        ...
        result["modules"]["jvm"] = f"ERROR: Java not found"
    ...
    try:
        ...
        try:
            ...
            result["setup_instructions"] = "MPXJ should be automatically available via the mpxj Python package. Try: pip install --force-reinstall mpxj"
...
def parse_mpp_file(file_path: str) -> Dict[str, Any]:
    ...
    def java_date_to_str(java_date):
        ...
        if hasattr(java_date, 'getYear') and hasattr(java_date, 'getMonthValue') and hasattr(java_date, 'getDayOfMonth'):
...
def parse_date_str(date_str) -> Optional[date]:
    ...
    if hasattr(date_str, 'getYear') and hasattr(date_str, 'getMonthValue') and hasattr(date_str, 'getDayOfMonth'):
        try:
            return date(
...
async def test_mpp_import(
    ...
    try:
        ...
        all_ok = (
            "ERROR" not in str(modules.get("jpype", "")) and
            "ERROR" not in str(modules.get("jvm", "")) and
            "ERROR" not in str(modules.get("mpxj", ""))
        )
        
        return {
            "success": all_ok,
            "ready": all_ok,
            "message": "MPP import is ready" if all_ok else "MPP import not available - see details below",
            **result
        }
...
async def import_project_full(
    ...
    try:
        ...
        if "libjvm" in error_msg or "JVMNotFoundException" in str(type(e).__name__) or "JAVA_HOME" in error_msg:
