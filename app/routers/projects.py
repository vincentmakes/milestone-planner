...
from app.schemas.project import (
    ...
    StaffAssignmentInPhase,
    ProjectStaffAssignmentResponse,
    EquipmentAssignmentInProject,
)
...
def build_subphase_tree_optimized(
    ...
    children_index: dict = None,
    staff_index: dict = None,
) -> List[dict]:
    """
    ...
    for sp in direct_children:
        ...
        staff_list = staff_index.get(sp.id, [])
        staff = [
            {
                "id": ssa.id,
        ...
        children.append({
            ...
            "children": build_subphase_tree_optimized(
                subphases, subphase_staff, sp.id, "subphase",
                children_index, staff_index
            ),
        })
    return children
...
def build_subphase_tree(
    ...
    return build_subphase_tree_optimized(
        subphases, subphase_staff, parent_id, parent_type
    )
...
async def get_projects(
    ...
    includeOtherSites: Optional[str] = Query(None),
...
async def get_project(
    ...
    # Log all query times
    print(f"PROJECT {project_id}: q1={int((t1-t0)*1000)}ms q2={int((t2-t1)*1000)}ms q3={int((t3-t2)*1000)}ms q4={int((t4-t3)*1000)}ms q5={int((t5-t4)*1000)}ms q6={int((t6-t5)*1000)}ms q7={int((t7-t6)*1000)}ms TOTAL={int((t7-t0)*1000)}ms")
    ...
    print(f"PROJECT {project_id} TIMING: q6={int((t6-t5)*1000)}ms q7={int((t7-t6)*1000)}ms proc={int((t8-t7)*1000)}ms TOTAL={int((t8-t0)*1000)}ms")
...
async def delete_project(
    ...
    await db.execute(
        ProjectPhase.__table__.delete().where(ProjectPhase.project_id == project_id)
    )
    await db.execute(
...
async def create_phase(
    ...
    if data.order_index is not None:
        ...
        max_result = await db.execute(
            select(func.max(ProjectPhase.sort_order))
            .where(ProjectPhase.project_id == project_id)
        )
...
async def create_subphase_under_phase(
    ...
    if data.order_index is not None:
        ...
        # Get max sort_order and append
        max_result = await db.execute(
            select(func.max(ProjectSubphase.sort_order))
            .where(and_(
                ProjectSubphase.parent_id == phase_id,
                ProjectSubphase.parent_type == "phase"
            ))
...
async def delete_subphase(
    ...
    await db.execute(
        SubphaseStaffAssignment.__table__.delete()
        .where(SubphaseStaffAssignment.subphase_id.in_(all_ids))
    )
    
    # Delete subphases (children first due to potential FK constraints)
    for sp_id in reversed(all_ids):
        await db.execute(
            ProjectSubphase.__table__.delete().where(ProjectSubphase.id == sp_id)
        )
