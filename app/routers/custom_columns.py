...
async def get_custom_columns(
    ...
    if site_id:
        stmt = select(CustomColumn).where(
            or_(
                CustomColumn.site_id.is_(None),
                CustomColumn.site_id == site_id
            )
...
async def create_custom_column(
    ...
    if data.column_type == "list":
        if not data.list_options or len(data.list_options) == 0:
            raise HTTPException(
                status_code=400,
                detail="list_options is required for list type columns"
            )
    ...
    if data.site_id:
        ...
        count_stmt = select(CustomColumn).where(
            or_(
                CustomColumn.site_id.is_(None),
                CustomColumn.site_id == data.site_id
            )
        )
    else:
    ...
    if existing_count >= MAX_CUSTOM_COLUMNS:
        raise HTTPException(
            status_code=400,
            detail=f"Maximum of {MAX_CUSTOM_COLUMNS} custom columns allowed"
        )
    
    # Get the next display order
    order_stmt = select(CustomColumn.display_order).order_by(
        CustomColumn.display_order.desc()
    ).limit(1)
...
async def update_custom_column(
    ...
    if data.list_options is not None:
        if column.column_type != "list":
            raise HTTPException(
                status_code=400,
                detail="list_options can only be set for list type columns"
            )
...
async def reorder_custom_columns(
    ...
    stmt = select(CustomColumn).where(
        CustomColumn.id.in_(data.column_order)
...
async def set_custom_column_value(
    ...
    if column.column_type == "boolean" and data.value is not None:
        if data.value.lower() not in ("true", "false"):
            raise HTTPException(
                status_code=400,
                detail="Boolean values must be 'true' or 'false'"
            )
