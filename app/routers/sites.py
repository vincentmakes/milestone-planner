...
async def add_custom_holiday(
    ...
    try:
        ...
        if "unique" in str(e).lower() or "duplicate" in str(e).lower():
            raise HTTPException(
                status_code=400,
                detail="A holiday with this name already exists on this date"
            )
        raise HTTPException(status_code=500, detail=str(e))
...
async def refresh_bank_holidays(
    ...
    if not site.country_code:
        raise HTTPException(
            status_code=400,
            detail="Site has no country code configured"
        )
            ...
            try:
                ...
                for h in holidays_data:
                    ...
                    if region_code and h.get("counties"):
                        ...
                        region_match = any(
                            region_code in county 
                            for county in h.get("counties", [])
                        )
                        if not region_match:
                            continue
