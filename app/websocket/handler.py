...
async def get_user_from_session(session_id: str, db: AsyncSession) -> Optional[User]:
    """
    ...
    try:
        ...
        if not session:
            print(f"[WS Auth] Session not found in database")
            return None
        
        print(f"[WS Auth] Session found, checking expiry...")
        ...
        print(f"[WS Auth] Session valid, parsing data...")
        ...
        if not user_id:
            print(f"[WS Auth] No user ID in session data")
            return None
...
def extract_session_id(cookie_value: str) -> Optional[str]:
    """
...
def get_tenant_from_scope(websocket: WebSocket) -> str:
    ...
    print(f"[WS get_tenant_from_scope] path={path}, state keys={list(state.keys())}, tenant_slug={tenant_slug}")
...
async def handle_websocket_connection(websocket: WebSocket, tenant_slug: str = None):
    """
