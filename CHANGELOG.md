...
- The percentage each assignee is booked at can be **changed from the card itself**, in the same 5% steps the Gantt chart's allocation slider offers, instead of only at the point of assigning. A booking above that person's own maximum capacity is flagged. Changing an allocation requires superuser rights — being assigned to a card lets you move it, not re-plan it.
